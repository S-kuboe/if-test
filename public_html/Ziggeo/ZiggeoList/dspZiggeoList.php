...
<html>
	<head>
		...
		<script>ZiggeoApi.token = "<?php echo $settings["token"] ?>";</script>		
		<?php
		require_once($settings["sdkpath"]);
				...
				<div class="container">
					...
					<?php if ( is_array( $video->tags ) && count( $video->tags ) > 0 ) { ?>
						...
						<?php if ( $settings["name_google"] ) { ?>
							<p><a href="http://google.com#q=<?php echo urlencode( $name ) ?>" target="_blank"><?php echo $name ?> &raquo;</a></p>
						<?php } else { ?>
							<p><?php echo $name ?></p>
						<?php } ?> 
					<?php } ?>
